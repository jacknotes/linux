...
SOURCE_DIRECTOR="/shell/template"
TARGET_DIRECTOR="/kuberentes_manifests"
...
check_endpoint(){
	...
	SECOND_NUMBER=20
	sleep 1
	if [[ `sudo kubectl get replicaset -o wide -n ${NAMESPACE} 2> /dev/null | sudo grep "${DEPLOYMENT_NAME}.*${IMAGE}" 2> /dev/null` ]];then
		REPLICASET_NAME=`sudo kubectl get replicaset -o wide -n ${NAMESPACE} | sudo grep "${DEPLOYMENT_NAME}.*${IMAGE}" 2> /dev/null | sudo awk '{print $1}'`
		echo "`eval ${DATETIME}`: replicaset: ${REPLICASET_NAME} is running, current 'READY' status is not READY, probe BEGIN."
		for i in `sudo seq 1 ${SECOND_NUMBER}`;do
			echo "`eval ${DATETIME}`: probe ${i} second..."	
			REPLICASET_RERULT=`sudo kubectl get replicaset -o wide -n ${NAMESPACE} | sudo grep "${DEPLOYMENT_NAME}.*${IMAGE}" | sudo awk '{if($2==$3 && $3==$4 && $2==replica) print "1"; else print "0";}' replica=$REPLICA`
...
create(){
	...
	#test namespace if legal
	sudo kubectl get ns/${NAMESPACE} >& /dev/null
	if [ $? != 0 ];then
		echo "`eval ${DATETIME}`: ERROR: namespace:${NAMESPACE} not found"
		exit 10
	fi

	##judge variable DEPLOY_TYPE if legal.
	if [[ ! `sudo kubectl get deployment -n ${NAMESPACE} 2> /dev/null | grep ${PROJECT_NAME}-base` ]];then
		DEPLOY_TYPE='base'
	elif [[ ! `sudo kubectl get deployment -n ${NAMESPACE} 2> /dev/null | grep ${PROJECT_NAME}-canary-weight` ]];then
	...
	[ ! -e ${TEMPLATE_DIRECTORY} ] && sudo mkdir -p ${TEMPLATE_DIRECTORY}
	...
	if [ ! -e ${DEPLOYMENT_SOURCE_TEMPLATE_FILE} ];then
		...
		if [ ${DEPLOY_TYPE} == 'base' ];then
			...
			sudo \cp -a ${DEPLOYMENT_SOURCE_TEMPLATE_FILE} ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
			sudo \cp -a ${INGRESS_SOURCE_TEMPLATE_FILE} ${INGRESS_TARGET_TEMPLATE_FILE}
			if [ $? == 0 ];then
	...
	sudo sed -i "s#__Service_metadata_name__#${Service_metadata_name}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Service_metadata_namespace__#${Service_metadata_namespace}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Service_spec_selector_app__#${Service_spec_selector_app}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Service_spec_selector_tag__#${Service_spec_selector_tag}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Service_spec_ports_targetPort__#${Service_spec_ports_targetPort}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	# modify deployment yaml section
	sudo sed -i "s#__Deployment_metadata_name__#${Deployment_metadata_name}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_metadata_namespace__#${Deployment_metadata_namespace}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_spec_replicas__#${Deployment_spec_replicas}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_spec_selector_matchLabels_app__#${Deployment_spec_selector_matchLabels_app}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_spec_selector_matchLabels_tag__#${Deployment_spec_selector_matchLabels_tag}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_spec_template_metadata_labels_app__#${Deployment_spec_template_metadata_labels_app}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_spec_template_metadata_labels_tag__#${Deployment_spec_template_metadata_labels_tag}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_spec_template_spec_containers_name__#${Deployment_spec_template_spec_containers_name}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s#__Deployment_spec_template_spec_containers_image__#${Deployment_spec_template_spec_containers_image}#g" ${DEPLOYMENT_TARGET_TEMPLATE_FILE} 
	if [ $? != 0 ];then
	...
	sudo sed -i "s,__Ingress_metadata_name__,${Ingress_metadata_name},g" ${INGRESS_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s,__Ingress_metadata_namespace__,${Ingress_metadata_namespace},g" ${INGRESS_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s,__Ingress_spec_rules_host__,${Ingress_spec_rules_host},g" ${INGRESS_TARGET_TEMPLATE_FILE} && \
	sudo sed -i "s,__Ingress_spec_rules_http_paths_backend_serviceName__,${Ingress_spec_rules_http_paths_backend_serviceName},g" ${INGRESS_TARGET_TEMPLATE_FILE}
	if [ $? != 0 ];then
	...
	sudo kubectl apply -f ${DEPLOYMENT_TARGET_TEMPLATE_FILE} -n ${NAMESPACE} &> /dev/null 
	if [ $? != 0 ];then
	...
	if [ $? != 0 ];then
		...
		sudo kubectl delete -f ${DEPLOYMENT_TARGET_TEMPLATE_FILE} -n ${NAMESPACE}
		exit 5
	else
	...
	if [ ${DEPLOY_TYPE} == "base" ];then
		sudo sed -i 's,nginx.ingress.kubernetes.io/canary,#nginx.ingress.kubernetes.io/canary,g' ${INGRESS_TARGET_TEMPLATE_FILE}
	fi
	#apply modify ingress template file.
	sudo kubectl apply -f ${INGRESS_TARGET_TEMPLATE_FILE} -n ${NAMESPACE} > /dev/null
        if [ $? != 0 ];then
        	echo "`eval ${DATETIME}`: apply ingress template file failure"
		#sudo kubectl delete -f ${INGRESS_TARGET_TEMPLATE_FILE} -n ${NAMESPACE}
		sudo kubectl delete -f ${DEPLOYMENT_TARGET_TEMPLATE_FILE} -n ${NAMESPACE}
        	exit 10
        else
		...
		if [ ${DEPLOY_TYPE} == "canary-weight" ];then
			...
			sudo kubectl patch deployment/${PROJECT_NAME}-base -p '{"spec": {"replicas": 1}}' -n ${NAMESPACE} 
		fi
        fi
...
deploy(){
	...
	INGRESS_WEIGHT_RESULT=`sudo kubectl get ingress/${INGRESS_CANARY_WEIGHT_NAME} -n ${NAMESPACE} -o yaml 2>  /dev/null | grep 'canary-weight: "' | awk -F '"' '{print $2}'`
	#patch directory
	...
	if [ ! -e ${TARGET_PATCH_DIRECTOR} ];then
		sudo mkdir -p ${TARGET_PATCH_DIRECTOR}
	fi
	...
	if [ ${INGRESS_WEIGHT_RESULT} == 0 ];then
		...
		if [ -e ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE} ];then
			sudo \cp -a ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE} ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE}-backup
		fi

		#copy source template to target template
		sudo \cp -a ${SOURCE_PATCH_DEPLOYMENT_FILE} ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE}
		if [ $? != 0 ];then
		...
		sudo sed -i "s#`cat ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE} | grep 'name: .*'`#      - name: ${PROJECT_NAME}#g" ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE}
		if [ $? == 0 ];then
		...
		sudo sed -i "s#`cat ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE} | grep 'image: .*'`#        image: ${IMAGE}#g" ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE}
		if [ $? == 0 ];then
		...
		sudo sed -i "s#`cat ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE} | grep 'replicas: .*'`#  replicas: ${REPLICA}#g" ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE}
		if [ $? == 0 ];then
		...
		sudo kubectl patch deployment/${DEPLOYMENT_CANARY_WEIGHT_NAME} -n ${NAMESPACE} -p "$(cat ${PATCH_DEPLOYMENT_CANARY_WEIGHT_FILE})" &> /dev/null
		if [ $? == 0 ];then
		...
		if [ $? == 0 ] ;then
			...
			if [ -e ${PATCH_INGRESS_CANARY_WEIGHT_FILE} ];then
				sudo \cp -a ${PATCH_INGRESS_CANARY_WEIGHT_FILE} ${PATCH_INGRESS_CANARY_WEIGHT_FILE}-backup
			fi

			#copy source template to target template
			sudo \cp -a ${SOURCE_PATCH_INGRESS_CANARY_WEIGHT_FILE} ${PATCH_INGRESS_CANARY_WEIGHT_FILE}
			if [ $? != 0 ];then
			...
			sudo sed -i "s#`cat ${PATCH_INGRESS_CANARY_WEIGHT_FILE} | grep 'nginx.ingress.kubernetes.io/canary-weight: .*' | awk -F ':' '{print $2}' | awk -F '"' '{print $2}'`#${WEIGHT_VALUE}#g" ${PATCH_INGRESS_CANARY_WEIGHT_FILE} 
			if [ $? == 0 ];then
			...
			sudo kubectl patch ingress/${INGRESS_CANARY_WEIGHT_NAME} -n ${NAMESPACE} -p "$(cat ${PATCH_INGRESS_CANARY_WEIGHT_FILE})" &> /dev/null
			if [ $? == 0 ];then
				...
				#patch not usage deployment replica is 1
				sleep 5
				sudo kubectl patch deployment/${DEPLOYMENT_BASE_NAME} -p '{"spec": {"replicas": 1}}' -n ${NAMESPACE}
				return 0
			else
	...
	# set ingress weight value is 0,disable ingress for base.
		...
		if [ -e ${PATCH_DEPLOYMENT_BASE_FILE} ];then
			sudo \cp -a ${PATCH_DEPLOYMENT_BASE_FILE} ${PATCH_DEPLOYMENT_BASE_FILE}-backup
		fi

		#copy source template to target template
		sudo \cp -a ${SOURCE_PATCH_DEPLOYMENT_FILE} ${PATCH_DEPLOYMENT_BASE_FILE}
		if [ $? != 0 ];then
		...
		# modify deployment file
		#1. modify container_name
		sudo sed -i "s#`cat ${PATCH_DEPLOYMENT_BASE_FILE} | grep 'name: .*'`#      - name: ${PROJECT_NAME}#g" ${PATCH_DEPLOYMENT_BASE_FILE}
		if [ $? == 0 ];then
			echo "`eval ${DATETIME}`: modify deployment file for args: 'container_name' successful"
		else
			echo "`eval ${DATETIME}`: modify deployment file for args: 'container_name' failure."
		...
		#2. modify container_image
		sudo sed -i "s#`cat ${PATCH_DEPLOYMENT_BASE_FILE} | grep 'image: .*'`#        image: ${IMAGE}#g" ${PATCH_DEPLOYMENT_BASE_FILE}
		if [ $? == 0 ];then
			echo "`eval ${DATETIME}`: modify deployment file for args: 'container_image' successful" 
		else
			echo "`eval ${DATETIME}`: modify deployment file for args: 'container_image' failure."
		...
		#3. modify pod replica
		sudo sed -i "s#`cat ${PATCH_DEPLOYMENT_BASE_FILE} | grep 'replicas: .*'`#  replicas: ${REPLICA}#g" ${PATCH_DEPLOYMENT_BASE_FILE}
		if [ $? == 0 ];then
			echo "`eval ${DATETIME}`: modify deployment file for args: 'pod replica' successful" 
		else
			echo "`eval ${DATETIME}`: modify deployment file for args: 'pod replica' failure."
		...
		sudo kubectl patch deployment/${DEPLOYMENT_BASE_NAME} -n ${NAMESPACE} -p "$(cat ${PATCH_DEPLOYMENT_BASE_FILE})" &> /dev/null
		if [ $? == 0 ];then
		...
		if [ $? == 0 ] ;then
			...
			if [ $? == 0 ];then
				...
				#patch not usage deployment replica is 1
				sleep 5
				sudo kubectl patch deployment/${DEPLOYMENT_CANARY_WEIGHT_NAME} -p '{"spec": {"replicas": 1}}' -n ${NAMESPACE}
				return 0
			else
				echo "`eval ${DATETIME}`: patch ingress/${INGRESS_CANARY_WEIGHT_NAME} failure."
				exit 10
		...
		echo "`eval ${DATETIME}`: INGRESS_WEIGHT_RESULT value is not legal, can INGRESS:${PROJECT_NAME}-canary-weight already deleted. you should delete DEPLOYMENT:${PROJECT_NAME}-canary-weight make it normal."
		exit 10
	fi
}
...
rollback(){
	...
	if [ ${INGRESS_WEIGHT_RESULT} == 0 ];then
		...
		sudo kubectl patch ingress/${INGRESS_CANARY_WEIGHT_NAME} -n ${NAMESPACE} -p "$(cat ${PATCH_INGRESS_CANARY_WEIGHT_FILE})" &> /dev/null
		if [ $? == 0 ];then
			ROLLBACK_IMAGE_VERSION=`sudo kubectl get deployment/${DEPLOYMENT_CANARY_WEIGHT_NAME} -n ${NAMESPACE} -o jsonpath='{.spec.template.spec.containers[0].image}'`
			ROLLBACK_IMAGE_REPLICAS=`sudo kubectl get deployment/${DEPLOYMENT_CANARY_WEIGHT_NAME} -n ${NAMESPACE} -o jsonpath='{.spec.replicas}'`
	...
	#set ingress weight value is 0,disable ingress for weight.
		...
		sudo kubectl patch ingress/${INGRESS_CANARY_WEIGHT_NAME} -n ${NAMESPACE} -p "$(cat ${PATCH_INGRESS_CANARY_WEIGHT_FILE})" &> /dev/null
		if [ $? == 0 ];then
			ROLLBACK_IMAGE_VERSION=`sudo kubectl get deployment/${DEPLOYMENT_BASE_NAME} -n ${NAMESPACE} -o jsonpath='{.spec.template.spec.containers[0].image}'`
			ROLLBACK_IMAGE_REPLICAS=`sudo kubectl get deployment/${DEPLOYMENT_BASE_NAME} -n ${NAMESPACE} -o jsonpath='{.spec.replicas}'`
			echo "`eval ${DATETIME}`: rollback successful."
			echo "----------------rollback info-------------------"
			echo "namespace: ${NAMESPACE}"
			echo "project_name: ${PROJECT_NAME}"
...
case $1 in 
	...
	*)
		echo "USAGE: $0 { create NAMESPACE PROJECT_NAME PROJECT_DOMAIN_NAME POD_IMAGE POD_PORT POD_REPLICA } | { rollback NAMESPACE PROJECT_NAME } | { check_endpoint NAMESPACE PROJECT_NAME IMAGE REPLICA DEPLOYMENT_NAME } | { deploy NAMESPACE PROJECT_NAME POD_IMAGE POD_REPLICA } "
		;;
esac
