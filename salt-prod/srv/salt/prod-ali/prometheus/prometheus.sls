...
prometheus-source-install:
  file.managed:
    - name: /usr/local/src/prometheus-2.19.3.linux-amd64.tar.gz
    - source: salt://prometheus/files/prometheus-2.19.3.linux-amd64.tar.gz
    - user: root
    - group: root
    - mode: 755
  cmd.run:
    - name: cd /usr/local/src && tar xf prometheus-2.19.3.linux-amd64.tar.gz -C /usr/local/ && chown -R prometheus:prometheus /usr/local/prometheus-2.19.3.linux-amd64/ && ln -s /usr/local/prometheus-2.19.3.linux-amd64 /usr/local/prometheus  
