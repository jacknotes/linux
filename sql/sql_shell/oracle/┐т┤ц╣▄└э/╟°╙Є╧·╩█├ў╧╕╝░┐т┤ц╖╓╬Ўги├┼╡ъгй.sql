...
begin
  ...
  insert into tmp_test2
    ...
    select area.name, ---区域
           ...
           /*sum(bv.QTY*P.RTLPRC) TotalAmt, ---门店库存额*/

           count(distinct (case when bv.qty > 0 and bv.store <>'1000000' then bv.gdgid end)) kcSKU,  --门店在库sku数
           sum(case when bv.qty > 0 and bv.store <>'1000000' then bv.qty end) kcqty ,    --门店库存数量 
           sum(case when bv.qty > 0 and bv.store <>'1000000' then (bv.qty*P.RTLPRC) end) kcAmt , --门店库存金额
