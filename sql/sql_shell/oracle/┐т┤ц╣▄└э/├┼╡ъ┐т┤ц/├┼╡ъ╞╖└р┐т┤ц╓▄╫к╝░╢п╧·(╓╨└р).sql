...
select BB.gid            --店铺gid    
      ...
      ,decode((sum(AA.salamt1)/7),0,0, round(sum(BB.kcAmt) / (sum(AA.salamt1)/7), 1)) days  --7天周转天数
      ,sum(AA.salamt2)/15 --15天日均销售额
      ,decode((sum(AA.salamt2)/15),0,0, round(sum(BB.kcAmt) / (sum(AA.salamt2)/15), 1)) days1 --15天周转天数 
      ,BB.province                      --省份
      ,BB.bsname                     --中类
      ,BB.ascode                      --品类代码
      ,BB.bscode                     --中类代码
      ...
      ,decode((sum(AA.salamt3)/30),0,0, round(sum(BB.kcAmt) / (sum(AA.salamt3)/30), 1)) days2 --30天周转天数
      ,BB.storelevel
      ,BB.storescale
        
from
(select snd ,sum(salqty1) salqty1 ,sum(salamt1) salamt1 ,sum(qtySKU1) qtySKU1 , sum(salqty2) salqty2 ,sum(salamt2) salamt2 ,sum(qtySKU2) qtySKU2 , sum(salqty3) salqty3 ,sum(salamt3) salamt3 ,sum(qtySKU3) qtySKU3 ,c_faseason ,ascode ,asname ,bscode ,bsname
from(
select sdrpts.snd
      ,sum(decode(sdrpts.cls,'零售',1,'批发',1,'成本差异',0,'成本调整',0,-1)*sdrpts.qty) salqty1
      ,sum(decode(sdrpts.cls,'零售',1,'批发',1,'成本差异',0,'成本调整',0,-1)*(sdrpts.amt+sdrpts.tax)) salamt1
  ...
  left join (select * from sdrpts 
             where sdrpts.fildate>=Bfiledate1 and sdrpts.fildate<=Efiledate
...
select sdrpts.snd
      ...
      ,0 salamt1
      ,0 qtySKU1
      ,sum(decode(sdrpts.cls,'零售',1,'批发',1,'成本差异',0,'成本调整',0,-1)*sdrpts.qty) salqty2
      ,sum(decode(sdrpts.cls,'零售',1,'批发',1,'成本差异',0,'成本调整',0,-1)*(sdrpts.amt+sdrpts.tax)) salamt2
  ...
  left join h4v_goodssort on goods.gid = h4v_goodssort.gid
  left join (select * from sdrpts 
             where sdrpts.fildate>=Bfiledate2 and sdrpts.fildate<=Efiledate
             and sdrpts.cls in ('零售','零售退','批发','批发退','成本差异','成本调整')
            ) sdrpts 
  on goods.gid=sdrpts.gdgid
  where faseason like '%' || goods.c_faseason || '%'
...
select sdrpts.snd
      ...
      ,0 salamt2
      ,0 qtySKU2
      ,sum(decode(sdrpts.cls,'零售',1,'批发',1,'成本差异',0,'成本调整',0,-1)*sdrpts.qty) salqty3
      ,sum(decode(sdrpts.cls,'零售',1,'批发',1,'成本差异',0,'成本调整',0,-1)*(sdrpts.amt+sdrpts.tax)) salamt3
  ...
  left join h4v_goodssort on goods.gid = h4v_goodssort.gid
  left join (select * from sdrpts 
             where sdrpts.fildate>=Bfiledate3 and sdrpts.fildate<=Efiledate
             and sdrpts.cls in ('零售','零售退','批发','批发退','成本差异','成本调整')
            ) sdrpts 
  on goods.gid=sdrpts.gdgid
  where faseason like '%' || goods.c_faseason || '%'
  ...
  ) group by snd ,c_faseason ,ascode ,asname ,bscode ,bsname)AA
...
group by store.gid
        ...
        ,store.storelevel
        ,store.storescale
) BB on AA.snd = BB.gid and AA.c_faseason = BB.c_faseason and AA.ascode = BB.ascode and AA.bscode = BB.bscode
 group by BB.gid                
         ,BB.code           
         ,BB.name           
         ,BB.areaname
...
<CRITERIALIST>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR><=</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2015.12.20</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
...
<SG>
  ...
  <LINE>
    ...
    <SGLINEITEM>2015.12.20</SGLINEITEM>
...
<SQLREPORT>
<RPTTITLE>门店品类库存周转及动销(中类)</RPTTITLE>
