...
from
(select snd ,sum(salqty1) salqty1 ,sum(salamt1) salamt1 ,sum(qtySKU1) qtySKU1 , sum(salqty2) salqty2 ,sum(salamt2) salamt2 ,sum(qtySKU2) qtySKU2 , sum(salqty3) salqty3 ,sum(salamt3) salamt3 ,sum(qtySKU3) qtySKU3 ,c_faseason ,sortcode
from(
...
select sdrpts.snd
  ...
  ) group by snd ,c_faseason ,sortcode )AA
...
group by store.gid
        ...
        ,store.storelevel
        ,store.storescale
 ) BB on AA.snd = BB.gid and AA.c_faseason = BB.c_faseason and AA.sortcode = BB.sortcode
 group by BB.gid            
         ,BB.code           
         ,BB.name           
         ,BB.areaname
...
<CRITERIALIST>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR><=</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2016.02.03</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
...
<SG>
  ...
  <LINE>
    ...
    <SGLINEITEM>2016.02.03</SGLINEITEM>
