...
group by goods.gid ,store.area,area.name,store.PROVINCE ) AA on goods.gid=AA.gid
...
group by businv.GDGID ,store.area,store.PROVINCE ) BB on goods.gid=BB.GDGID
and AA.PROVINCE=BB.PROVINCE
...
commit;
  ...
  insert into tmp_test1
   ...
   select goods.sort, sum(Qty) CQty,goods.isnew
             from goods, businv
            where goods.gid = businv.GDGID
              and  businv.WRH = 1000020
            group by goods.sort,goods.isnew;
  commit;
...
<COLUMNLIST>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>case when tmp_test2.char9='否' then '老品' when tmp_test2.char9='是' then '新品'
else '' end</COLUMN>
    <TITLE>新品标记</TITLE>
    <FIELDTYPE>1</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
  ...
  <COLUMNITEM>
    <AGGREGATION>SUM</AGGREGATION>
    <COLUMN>decode((select sum(num2) from tmp_test2 t where t.char12=tmp_test2.char12),0,0,
 tmp_test2.num2*100/(select sum(num2) from tmp_test2 t where t.char12=tmp_test2.char12)
)</COLUMN>
    <TITLE>新老品销售额占比</TITLE>
    <FIELDTYPE>6</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
    <GAGGREGATION>SUM</GAGGREGATION>
    <DISPLAYINNEXT>TRUE</DISPLAYINNEXT>
...
<CRITERIALIST>
  ...
  <CRITERIAITEM>
    ...
    <PICKVALUE>
      <PICKVALUEITEM>select PROVINCE from PROVINCE
union 
select '' from dual
order by PROVINCE desc</PICKVALUEITEM>
