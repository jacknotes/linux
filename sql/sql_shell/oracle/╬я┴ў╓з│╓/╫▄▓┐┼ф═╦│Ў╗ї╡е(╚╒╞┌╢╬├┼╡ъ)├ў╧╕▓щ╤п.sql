...
BEGIN
  ...
  COMMIT;
    ---插入统配出数据
  INSERT INTO H4RTMP_PHMX(FMODIFIER,FTIME,STIME,NUM,CLS,GDCODE,GDNAME,GDCODE2,STNAME,QTY,QTYSTR,PRICE,
TOTAL,TAX,CAMT,CTAX,RTOTAL,STATNAME,FILDATE,LINE,STCODE,stdshow,lowinv,highinv,
alcline,stdshow0)
    SELECT  (select modifier
          from STKOUTLOG p
         where p.num = stkoutlog.num
           and p.cls = stkoutlog.cls
           and p.stat = 0),(select time from stkoutlog g where g.num =stkoutlog.num and g.cls = stkoutlog.cls and g.stat = 0), stkoutlog.time,STKOUT.NUM , STKOUT.CLS , GOODSH.CODE , GOODSH.NAME , GOODSH.CODE2 , 
...
alcline,stdshow0)
    SELECT  (select modifier
         ...
         where p.num = STKOUTLOG.num
           and p.cls = STKOUTLOG.cls
           and p.stat = 0), (select time from stkoutbcklog g where g.num =STKOUTLOG.num and g.cls = STKOUTLOG.cls and g.stat = 0), STKOUTLOG.time,STKOUT.NUM , STKOUT.CLS , GOODSH.CODE , GOODSH.NAME , GOODSH.CODE2 , 
      STORE.NAME , STKOUTDTL.QTY , STKOUTDTL.QTYSTR , 
      STKOUTDTL.PRICE , STKOUTDTL.TOTAL , STKOUTDTL.TAX , STKOUTDTL.CAMT , 
      STKOUTDTL.CTAX , STKOUTDTL.CRTOTAL , MODULESTAT.STATNAME,STKOUTLOG.TIME,
...
alcline,stdshow0)
    SELECT  (select modifier
          from diralcLOG p
         where p.num = diralcLOG.num
           and p.cls = diralcLOG.cls
           and p.stat = 0),(select time from diralcLOG g where g.num =diralcLOG.num and g.cls = diralcLOG.cls and g.stat = 0), diralcLOG.time,diralc.NUM , diralc.CLS , GOODSH.CODE , GOODSH.NAME , GOODSH.CODE2 , 
...
<COLUMNLIST>
  ...
  <COLUMNITEM>
    ...
    <DISPLAYINNEXT>TRUE</DISPLAYINNEXT>
    <ISKEY>FALSE</ISKEY>
    <COLUMNNAME>FMODIFIER</COLUMNNAME>
    <CFILTER>FALSE</CFILTER>
    <CFONTCOLOR>0</CFONTCOLOR>
    <CDISFORMAT></CDISFORMAT>
    <CGROUPINDEX>-1</CGROUPINDEX>
...
<CRITERIALIST>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR>>=</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2015.03.21</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR><</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2015.03.22</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
...
<SG>
  ...
  <LINE>
    ...
    <SGLINEITEM>2015.03.21</SGLINEITEM>
    <SGLINEITEM>2015.03.22</SGLINEITEM>
