...
declare
 ...
 commit;
  ...
  (CODE, bgndate ,enddate ,alcprc  ,lstinprc ,  total ,rtotal,/*alcfee ,*/
	lstintotal, invtotal, maoli,
	suiyi, cls, settleno, num,fildate)
	SELECT CLIENTH.GID, vbgndate, venddate, round(d.price/d.qpc,4) alcprc, d.lstinprc, SUM(D.TOTAL), SUM(D.RTOTAL), 
		sum(decode(d.lstinprc,0,g.cntinprc,d.lstinprc)*d.qty), sum(d.camt+d.ctax), sum(d.total - decode(d.lstinprc,0,g.cntinprc,d.lstinprc) * d.qty),
		sum(decode(d.lstinprc,0,g.cntinprc,d.lstinprc) * d.qty - (d.camt+d.ctax)), '批发', m.settleno, m.num,mm.time
	...
	GROUP BY CLIENTH.GID, round(d.price/D.qpc,4), d.lstinprc, vbgndate, venddate, m.settleno, m.num,mm.time;
	commit;
-----配货退货汇总
	insert into   H4rTMP_alcstkout
	(CODE, bgndate ,enddate ,alcprc  ,lstinprc ,  total ,rtotal, 
		lstintotal, invtotal,
		maoli,suiyi,
		cls, settleno, num,fildate)
	SELECT CLIENTH.GID, vbgndate, venddate, round(d.price / d.qpc, 4) alcprc, d.lstinprc, -SUM(D.TOTAL), -SUM(D.RTOTAL),
		-sum(decode(d.lstinprc,0,g.cntinprc,d.lstinprc)* d.qty), -sum(d.camt+d.ctax),
		-sum(d.total - decode(d.lstinprc,0,g.cntinprc,d.lstinprc) * d.qty), -sum(decode(d.lstinprc,0,g.cntinprc,d.lstinprc)* d.qty - (d.camt+d.ctax)),
	'批发退', m.settleno, m.num,mm.time
	...
	(code, bgndate, enddate, total, rtotal, lstintotal, invtotal, maoli, suiyi, cls, settleno, num,fildate)
	select code, bgndate, enddate, sum(total), sum(rtotal), sum(lstintotal), sum(invtotal), sum(maoli), SUM(suiyi), cls, settleno, num,fildate
    from H4rTMP_alcstkout
	group by code, bgndate, enddate, cls,settleno, num,fildate;
	commit;
end;
</BEFORERUN>
<AFTERRUN>
...
<COLUMNLIST>
  ...
  <COLUMNITEM>
    ...
    <COLUMN>H4rTMP_alcstkout3.FILDATE</COLUMN>
    <TITLE>实际记帐日期</TITLE>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>H4rTMP_alcstkout3.num</COLUMN>
    <TITLE>单号</TITLE>
    <FIELDTYPE>1</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
    <GAGGREGATION></GAGGREGATION>
...
<CRITERIALIST>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR><</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2013.03.18</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
...
<SG>
  ...
  <LINE>
    ...
    <SGLINEITEM>2013.03.18</SGLINEITEM>
    <SGLINEITEM></SGLINEITEM>
  </LINE>
  <LINE>
