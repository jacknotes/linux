...
group by s.code, s.name, t.code, t.name, to_char(b1.busdate, 'YYYY.MM.DD'),c.code,c.name,e.name||'['||e.code||']';
 ...
 s2,Dcode,RTNRECEIPT, S4,Adate,Emp)
  select s.code, s.name, t.code, t.name, to_char(pd.thedate, 'YYYY.MM.DD'), c.code,
 c.name, 0, 
  sum(pd.amount), max(pd.bankcode),max(pd.rtnreceipt), count(1), 
to_char(pd.thedate, 'YYYY.MM.DD'),p.operator
  ...
  and p.thedate >= BDate
  and p.thedate < EDate
  group by s.code, s.name, t.code, t.name, to_char(pd.thedate, 'YYYY.MM.DD'), 
...
<COLUMNLIST>
  ...
  <COLUMNITEM>
    <AGGREGATION>SUM</AGGREGATION>
    <COLUMN>(H4RTMP_JKDTL.S2)+nvl((select p.TOTAL from PS4STOREPAYMENTADJ p ,PS4STOREPAYMENTADJDTL pd where p.store =(select st.gid from store st where st.code = H4RTMP_JKDTL.CODE) and p.num = pd.num and p.stat = 300 and pd.currency = (select c.code from currency c where c.name = H4RTMP_JKDTL.CNAME) and pd.thedate =H4RTMP_JKDTL.Fildate ),0)</COLUMN>
    <TITLE>调整后缴款金额</TITLE>
    <FIELDTYPE>6</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
    <GAGGREGATION>SUM</GAGGREGATION>
...
<CRITERIALIST>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR>>=</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2014.12.28</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR><</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2015.01.04</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
  ...
  <CRITERIAITEM>
    ...
    <ISQUOTED>0</ISQUOTED>
    <PICKNAME>
      <PICKNAMEITEM>长款</PICKNAMEITEM>
      <PICKNAMEITEM>短款</PICKNAMEITEM>
      <PICKNAMEITEM>平</PICKNAMEITEM>
      <PICKNAMEITEM>没有销售传回</PICKNAMEITEM>
    </PICKNAME>
    <PICKVALUE>
      <PICKVALUEITEM>长款</PICKVALUEITEM>
      <PICKVALUEITEM>短款</PICKVALUEITEM>
      <PICKVALUEITEM>平</PICKVALUEITEM>
      <PICKVALUEITEM>没有销售传回</PICKVALUEITEM>
    </PICKVALUE>
    <DEFAULTVALUE></DEFAULTVALUE>
    <ISREQUIRED>FALSE</ISREQUIRED>
    <WIDTH>0</WIDTH>
...
<SG>
  ...
  <LINE>
    ...
    <SGLINEITEM>2014.12.28</SGLINEITEM>
    <SGLINEITEM>2015.01.04</SGLINEITEM>
...
<DXDBGRIDITEM>
  ...
  <DXFILTERLIST>
    ...
    <DXFILTERITEM>
      ...
      <DXFILTERCOLOR>8421440</DXFILTERCOLOR>
    </DXFILTERITEM>
  </DXFILTERLIST>
