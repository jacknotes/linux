...
(
  ...
  MAXQTY    NUMBER(24,4) default 0 not null,--限制商品总数
  ALLQTY    NUMBER(24,4) default 0 not null,--类别总品项数
  NEWQTY    NUMBER(24,4) default 0 not null,--新品品项数
  NEWRAT    NUMBER(24,4) default 0 not null,--新品引进率
  ALLINV    NUMBER(24,4) default 0 not null,--类别总库存数
  NEWINV    NUMBER(24,4) default 0 not null,--新品总库存数
  ALLRTOTAL NUMBER(24,4) default 0 not null,--类别销售总额
  NEWRTOTAL NUMBER(24,4) default 0 not null,--新品销售总额
  NEWSALQTY NUMBER(24,4) default 0 not null,--新品销售数量
  NEWSINV    NUMBER(24,4) default 0 not null,--门店库存数量
  NEWCINV    NUMBER(24,4) default 0 not null--配送中心总库存数
)
...
select v.code  from psvdremp p ,employee e ,vendor v where p.vendor = v.gid and p.empgid = e.gid and e.code = VUSERCODE;
  ...
  select s.scode,s.sname,nvl(s.maxqty,0),sum(nvl(g.allqty,0)),0,sum(nvl(g.allinv,0)),0,sum(nvl(r.saleamt90+r.saletax90,0)),0
  from sortname s,
       (select sortcode,count(distinct gdgid) allqty,sum(allinv) allinv from H4RTMP_NEWGOODS_2 group by sortcode) g,
       (select * from rpt_ctgSalDrpt where fildate = vDate)  r
   where s.scode = substr(g.sortcode(+),1,h4rcalc_basic.h4rcalc_sortCLength) 
   and s.scode = substr(r.ctgkey(+),1,h4rcalc_basic.h4rcalc_sortCLength)  --小类
   and s.acode = '0000' 
   group by s.scode,s.sname,nvl(s.maxqty,0) ;
   
  --汇总
  insert into H4RTMP_NEWGOODS(GDCODE,GDNAME,IMPORTDATE,
                                SORTCODE,SORTNAME,MAXQTY,ALLQTY,NEWQTY,ALLINV,NEWINV,ALLRTOTAL,NEWRTOTAL,NEWSALQTY,CREATEDATE,
                                NEWSINV ,NEWCINV)
  select g.GDCODE,g.GDNAME,g.IMPORTDATE, 
        s.scode,s.sname,nvl(s.maxqty,0),t.allqty,count(distinct g.gdgid),t.allinv,sum(g.allinv),t.allrtotal,sum(g.newrtotal),sum(g.newsalqty),vDate,
        sum(g.NEWSINV) ,sum(g.NEWCINV)
  from H4RTMP_NEWGOODS_2 g,sortname s,H4RTMP_NEWGOODS_1 t
   where substr(g.sortcode,1,h4rcalc_basic.h4rcalc_sortCLength) = s.scode --小类
   and s.scode = t.sortcode
   and s.acode = '0000'
   and g.createdate between vDate - vdiff and vDate+1 --商品资料的创建时间精确到时分秒 
   group by g.GDCODE,g.GDNAME,g.IMPORTDATE, 
            s.scode,s.sname,nvl(s.maxqty,0),
            t.allqty,t.allinv,t.allrtotal;   
...
<JOINLIST>
  ...
  <JOINITEM>
    ...
    <OPERATOR>=</OPERATOR>
    <RIGHT>GDVDR.VDRGID</RIGHT>
  </JOINITEM>
  <JOINITEM>
    <LEFT>GOODS.CODE</LEFT>
    <OPERATOR>=</OPERATOR>
    <RIGHT>H4RTMP_NEWGOODS.GDCODE</RIGHT>
  </JOINITEM>
  <JOINITEM>
    <LEFT>GOODS.GID</LEFT>
    <OPERATOR>=</OPERATOR>
    <RIGHT>GDVDR.GDGID</RIGHT>
  </JOINITEM>
</JOINLIST>
<COLUMNLIST>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>H4RTMP_NEWGOODS.GDCODE</COLUMN>
    <TITLE>商品代码</TITLE>
    <FIELDTYPE>1</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
    <GAGGREGATION></GAGGREGATION>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>H4RTMP_NEWGOODS.GDNAME</COLUMN>
    <TITLE>商品名称</TITLE>
    <FIELDTYPE>1</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
    <GAGGREGATION></GAGGREGATION>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>H4RTMP_NEWGOODS.IMPORTDATE</COLUMN>
    <TITLE>引进天数</TITLE>
    ...
    <DISPLAYINNEXT>TRUE</DISPLAYINNEXT>
    <ISKEY>FALSE</ISKEY>
    <COLUMNNAME>IMPORTDATE</COLUMNNAME>
    <CFILTER>FALSE</CFILTER>
    <CFONTCOLOR>0</CFONTCOLOR>
    <CDISFORMAT></CDISFORMAT>
    <CGROUPINDEX>-1</CGROUPINDEX>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>H4RTMP_NEWGOODS.NEWSALQTY</COLUMN>
    <TITLE>销售数量</TITLE>
    ...
    <DISPLAYINNEXT>TRUE</DISPLAYINNEXT>
    <ISKEY>FALSE</ISKEY>
    <COLUMNNAME>NEWSALQTY</COLUMNNAME>
    <CFILTER>FALSE</CFILTER>
    <CFONTCOLOR>0</CFONTCOLOR>
    <CDISFORMAT></CDISFORMAT>
    <CGROUPINDEX>-1</CGROUPINDEX>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>H4RTMP_NEWGOODS.NEWCINV</COLUMN>
    <TITLE>配送中心库存</TITLE>
    <FIELDTYPE>6</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
    <GAGGREGATION></GAGGREGATION>
  ...
  <COLUMNITEM>
    <AGGREGATION></AGGREGATION>
    <COLUMN>H4RTMP_NEWGOODS.NEWSINV</COLUMN>
    <TITLE>门店库存</TITLE>
    <FIELDTYPE>6</FIELDTYPE>
    <VISIBLE>TRUE</VISIBLE>
    <GAGGREGATION></GAGGREGATION>
...
<CRITERIALIST>
  ...
  <CRITERIAITEM>
    ...
    <OPERATOR><=</OPERATOR>
    <RIGHT>
      <RIGHTITEM>2015.01.22</RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
      <RIGHTITEM></RIGHTITEM>
    </RIGHT>
...
<SG>
  ...
  <LINE>
    ...
    <SGLINEITEM>2015.01.22</SGLINEITEM>
