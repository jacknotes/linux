...
begin
  ...
  from sortname s,
        (select substr(sortcode,1,h4rcalc_basic.h4rcalc_sortCLength) sortcode,count(distinct gdgid) allqty,sum(allinv) allinv 
          from H4RTMP_NEWGOODS_2 group by substr(sortcode,1,h4rcalc_basic.h4rcalc_sortCLength)) g,
        (select substr(ctgkey,1,h4rcalc_basic.h4rcalc_sortCLength) ctgkey,sum(nvl(saleamt90+saletax90,0)) saletotal90
           from rpt_ctgSalDrpt where fildate = vDate group by substr(ctgkey,1,h4rcalc_basic.h4rcalc_sortCLength))  r
   where s.scode = g.sortcode(+)
   and s.scode =  r.ctgkey(+) --小类
   and length(s.scode) = h4rcalc_basic.h4rcalc_sortCLength
   and s.acode = '0000' 
...
<NCRITERIAS>
  ...
  <NCRITERIALIST>
    <NEXTQUERY>新品销售情况分析.sql</NEXTQUERY>
    <NCRITERIAITEM>
      <LEFT>建档日期小于等于</LEFT>
