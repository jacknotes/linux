...
select up.cmpid as 单位编号,up.Name as 单位名称,case Enabled when 1 then '是' else '否' end as 是否开通网单,COUNT(*) as up
into #up
...
group by up.cmpid,up.Name,Enabled
order by Cmpid
...
select p3.单位编号,p3.单位名称,p3.差旅业务顾问,p3.运营经理,售后主管,维护人,开发人,isnull(up,0) as 账号数,isnull(a1.下单数,0) as 机票下单数,ISNULL(h1.下单数,0) as 酒店下单数,isnull(关注数,0) as 微信关注数,up.是否开通网单
into #p4
from #p3 p3
...
select p3.单位编号,p3.单位名称,p3.差旅业务顾问,p3.运营经理,售后主管,维护人,开发人,isnull(up,0) as 账号数,isnull(a2.下单数,0) as 机票下单数,ISNULL(h2.下单数,0) as 酒店下单数,isnull(关注数,0) as 微信关注数,up.是否开通网单
into #p5
from #p3 p3
inner join #up up on up.单位编号=p3.单位编号
...
select p4.单位编号,p4.单位名称,p4.机票下单数+p4.酒店下单数 as 线上订单数,p4.机票下单数+p4.酒店下单数+p5.机票下单数+p5.酒店下单数 as 全部订单数,p4.售后主管,p4.维护人,p4.开发人,p4.微信关注数,p4.差旅业务顾问,p4.运营经理,p4.是否开通网单
into #p10
from #p4 p4
...
left join  homsomDB..Trv_ItktBookings t2 on t1.ID=t2.TravelID
left join homsomDB..Trv_TktBookings t3 on t2.ID=t3.ID
where t3.AdminStatus in (2,3,4,5) and t3.CreateDate>='2018-11-01' and t3.CreateDate<'2018-11-03'
AND BookingSource IN(1,5,10,11) 
group by uc.Cmpid,uc.Name
...
select ROW_NUMBER() over(order by uc.Cmpid) as 序列号,
	...
	from homsomDB..Trv_UnitCompanies uc 	
left join [HotelOrderDB].[dbo].[HTL_Orders] a on a.CMPID=uc.Cmpid
where a.CreateDate>= '2018-11-01' and a.CreateDate<='2018-11-03'  and a.Status IN (40,92)
and Resource in (1,3)
group by uc.Cmpid,uc.Name
...
select p3.单位编号,p3.单位名称,p3.差旅业务顾问,p3.运营经理,售后主管,维护人,开发人,isnull(up,0) as 账号数,isnull(a3.下单数,0) as 机票下单数,ISNULL(h3.下单数,0) as 酒店下单数,isnull(关注数,0) as 微信关注数,up.是否开通网单
into #p6
from #p3 p3
inner join #up up on up.单位编号=p3.单位编号
...
select p3.单位编号,p3.单位名称,p3.差旅业务顾问,p3.运营经理,售后主管,维护人,开发人,isnull(up,0) as 账号数,isnull(a4.下单数,0) as 机票下单数,ISNULL(h4.下单数,0) as 酒店下单数,isnull(关注数,0) as 微信关注数,up.是否开通网单
into #p7
from #p3 p3
inner join #up up on up.单位编号=p3.单位编号
...
select p6.单位编号,p6.单位名称,p6.机票下单数+p6.酒店下单数 as 线上订单数,p6.机票下单数+p6.酒店下单数+p7.机票下单数+p7.酒店下单数 as 全部订单数,p6.售后主管,p6.维护人,p6.开发人,p6.微信关注数,p6.差旅业务顾问,p6.运营经理,p6.是否开通网单
into #p11
from #p6 p6
...
select p10.单位编号,p10.单位名称,p10.差旅业务顾问,p10.运营经理,p10.售后主管,p10.维护人,p10.开发人,p10.线上订单数 as '10月线上订单数',p10.全部订单数 as '10月全部订单数',p11.线上订单数 as '11月1-2日线上订单数'
,p11.全部订单数 as '11月1-2日全部订单数',p11.微信关注数,p10.是否开通网单 
from #p10 p10
left join #p11 p11 on p11.单位编号=p10.单位编号
where p10.全部订单数<>0 and p10.是否开通网单='否'
