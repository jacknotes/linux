...
select t1.单位编号,MONTH(t3.datetime) as 月
...
where  (t3.datetime>='2018-01-01' and t3.datetime<'2018-08-01')
and 单位编号<>''
group by t1.单位编号,MONTH(t3.datetime)
order by t1.单位编号
...
select t1.单位编号,MONTH(prdate) as 月
...
where 
(h1.prdate>='2018-01-01' and h1.prdate<'2018-08-01') and 
status !='-2'
and 单位编号<>''
group by t1.单位编号,MONTH(prdate)
order by t1.单位编号
...
select t1.单位编号,MONTH(bldate) as 月
...
where 
(h3.bldate>='2018-01-01' and h3.bldate<'2018-08-01') and 
status !='-2'
and 单位编号<>''
group by t1.单位编号,MONTH(bldate)
order by t1.单位编号
...
select t1.单位编号,MONTH(l1.OperDate) as 月
...
where 
 (l1.OperDate>='2018-01-01' and l1.OperDate<'2018-08-01')
and 单位编号<>''
group by t1.单位编号,MONTH(l1.OperDate)
order by t1.单位编号
...
select t1.单位编号,MONTH(c1.OperDate) as 月
...
where 
 (c1.OperDate>='2018-01-01' and c1.OperDate<'2018-08-01')
and 单位编号<>''
group by t1.单位编号,MONTH(c1.OperDate)
order by t1.单位编号
...
select t1.单位编号,MONTH(trainO.CreateDate) as  月
...
where 
(trainO.CreateDate>='2018-01-01' and trainO.CreateDate<'2018-08-01')
and (r.AuditTime >='2018-01-01' AND r.AuditTime<'2018-08-01')
and trainO.Isdisplay=0
and 单位编号<>''
group by t1.单位编号,MONTH(trainO.CreateDate)
order by t1.单位编号
...
IF OBJECT_ID('tempdb.dbo.#z') IS NOT NULL DROP TABLE #z
select cmp.单位编号,单位名称,indate,tb.月
,isnull(tb1,0)+isnull(hb1,0)+isnull(zb1,0)+isnull(lb1,0)+isnull(cb1,0)+isnull(fb1,0) as 销量
...
left join #hb hb on hb.单位编号=cmp.单位编号 and tb.月=hb.月
left join #zb zb on zb.单位编号=cmp.单位编号 and zb.月=tb.月 and zb.月=hb.月
left join #lb lb on lb.单位编号=cmp.单位编号 and lb.月=tb.月 and lb.月=hb.月 and lb.月=zb.月
left join #cb cb on cb.单位编号=cmp.单位编号 and cb.月=tb.月 and cb.月=hb.月 and cb.月=zb.月 and cb.月=lb.月
left join #fb fb on fb.单位编号=cmp.单位编号 and fb.月=tb.月 and fb.月=hb.月 and fb.月=zb.月 and fb.月=lb.月 and fb.月=cb.月
